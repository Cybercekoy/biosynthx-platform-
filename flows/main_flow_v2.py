...
def generate_dataset(n:int=1000, t1d_ratio:float=0.35, seed:int=42) -> pd.DataFrame:
    ...
    a1c  = (dx == "T1D") * rng.normal(8.1, 1.2, size=n) + (dx == "T2D") * rng.normal(7.6, 1.1, size=n)
    a1c  = np.clip(a1c, 5.0, 14.0)
    insulin = (dx == "T1D") & (rng.random(size=n) < 0.92) | ((dx == "T2D") & (rng.random(size=n) < 0.28))
    cgm_days = np.where(dx=="T1D", rng.integers(15, 30, size=n), rng.integers(0, 20, size=n))
    df = pd.DataFrame({
